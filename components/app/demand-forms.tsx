...
export function MineForm({ siteId, suggestedSeeds, locale = { country: "us", language: "en" } }: { siteId: string; suggestedSeeds: string[]; locale?: { country: string; language: string } }) {
  const [state, action, pending] = useActionState<ActionResult | null, FormData>(mineDemandAction, null);
  return (
    <form action={action} className="grid gap-3">
      <LiveRefresh active={!!state?.ok} intervalMs={8000} maxMs={5 * 60 * 1000} />
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <div className="grid gap-1"><Label htmlFor="mine-country">Country</Label><Input id="mine-country" name="country" defaultValue={locale.country} maxLength={2} /></div>
        <div className="grid gap-1"><Label htmlFor="mine-language">Language</Label><Input id="mine-language" name="language" defaultValue={locale.language} maxLength={5} /></div>
      ...
      <div className="flex items-center gap-3">
        ...
        {state ? <span className={`text-xs ${state.ok ? "text-muted-foreground" : "text-destructive"}`}>{state.ok ? "Queued. Autocomplete and People-Also-Ask expansion runs in the background; the Questions tab updates as results land." : state.error}</span> : null}
      </div>
    </form>
  );
}

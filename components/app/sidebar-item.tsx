 ...
 */
export function SidebarItem({ href, label, icon, sections, current, section }: { href: string; label: string; icon: React.ReactNode; sections?: NavSection[]; current: boolean; section?: string }) {
  ...
  return (
    <li>
      <div className={`flex items-center rounded-md ${current ? "bg-background font-medium shadow-xs" : "text-muted-foreground hover:bg-background/60 hover:text-foreground"}`}>
        <Link href={href as Route} aria-current={current ? "page" : undefined} className="flex min-w-0 flex-1 items-center gap-2 px-2 py-1.5 text-sm">

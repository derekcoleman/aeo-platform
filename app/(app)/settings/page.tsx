...
import { SETTINGS_SECTIONS, resolveTab, settingsHref } from "@/lib/app/nav";
...
import { listOrganizations, loadSite } from "@/lib/app/store";
import { requireUser, roleIn, visibleOrgIds } from "@/lib/auth/session";
...
/**
 * The settings hub: one organisation's members, plan, options and audit
 * trail, kept apart from the project pages. `?site=` keeps that project in
 * the sidebar and selects its organisation; `?org=` selects one directly;
 * neither picks the caller's first. Legacy `?tab=settings` still lands on
 * the organisation tab.
 */
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ checkout?: string; tab?: string; site?: string; org?: string }> }) {
  const { checkout, tab: requestedTab, site: siteParam, org: orgParam } = await searchParams;
  const user = await requireUser(settingsHref(siteParam, orgParam));
  ...
  if (!orgId) {
    return (
      <AppShell user={user} active="settings" page="settings">
        ...
        <Card><CardHeader><CardTitle>No organisation yet</CardTitle><CardDescription>Create one from the <Link className="underline-offset-2 hover:underline" href={"/app" as Route}>projects page</Link>; its members, plan and options will live here.</CardDescription></CardHeader></Card>
      </AppShell>
    );
  }
  ...
  const legacyTab = requestedTab === "settings" ? "organisation" : requestedTab;
  const tab = resolveTab(SETTINGS_SECTIONS, legacyTab, checkout ? "billing" : "members");
  ...
  return (
    <AppShell user={user} active="settings" site={siteForOrg} page="settings" section={tab}>
      <PageHeader title="Settings" eyebrow={org.name} description={`${org.plan} (${org.plan_status}) · ${members.length} member${members.length === 1 ? "" : "s"}`}>
        ...
        <div className="text-muted-foreground mb-6 flex flex-wrap items-center gap-2 text-sm">
          ...
          {orgs.slice(0, ORG_PICKER_MAX).map((o) => (
            o.id === orgId
              ...
              : <Link key={o.id} href={settingsHref(null, o.id) as Route} className="hover:text-foreground underline-offset-2 hover:underline">{o.name}</Link>
          ))}
      ...
      <UrlTabs defaultValue={checkout ? "billing" : "members"} values={SETTINGS_SECTIONS.map((s) => s.value)}>
        <TabsList>
          {SETTINGS_SECTIONS.map((s) => <TabsTrigger key={s.value} value={s.value}>{s.label}</TabsTrigger>)}
        </TabsList>
        ...
        <TabsContent value="organisation" className="grid gap-4 pt-4">
          <Card>

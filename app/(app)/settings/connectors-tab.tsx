...
/**
 * Settings → Connectors: a compact grid of tiles, one per connector the
 * product offers for the current project, connected or not. A tile opens a
 * dialog with the rows that exist, their sync state, and the way to connect
 * or finish setting up. Connectors are set per project, so without one the
 * tab lists the organisation's projects to pick from.
 */
export async function ConnectorsTab({ site, orgSites, manage, notice }: { site: SiteRow | null; orgSites: SiteRow[]; manage: boolean; notice: { connector?: string; error?: string; connected?: string } }) {
  if (!site) {
    return (
      <Card>
        <CardHeader><CardTitle>Choose a project</CardTitle><CardDescription>Connectors are set per project: each one reads from and publishes to its own domain. Organisation-wide sources (Slack, custom sources) show on every project.</CardDescription></CardHeader>
        <CardContent>
          {orgSites.length === 0 ? <p className="text-muted-foreground text-sm">This organisation has no projects yet. Create one from <Link className="underline-offset-2 hover:underline" href={"/app" as Route}>Projects</Link>.</p> : (
            <ul className="grid gap-2 sm:grid-cols-2">
              {orgSites.map((s) => (
                <li key={s.id}><Link href={settingsHref(s.id, null, "connectors") as Route} className="hover:bg-muted/60 block rounded-lg border px-3 py-2"><span className="block text-sm font-medium">{s.name}</span><span className="text-muted-foreground block truncate text-xs">{s.canonical_domain}{s.path_prefix}</span></Link></li>
              ))}
            </ul>
          )}
        </CardContent>
  ...
  const overview = await connectorsOverview(site.org_id, { siteId: site.id });
  ...
  const { connector, error, connected } = notice;
  return (
    <div className="max-w-3xl">
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <p className="text-muted-foreground text-sm">What <span className="text-foreground font-medium">{site.name}</span> reads from and publishes to. Click a tile to connect it or finish setting it up.</p>
...
function ConnectPanel({ entry, site, sites, overview }: { entry: CatalogEntryView; site: SiteRow; sites: SiteOption[]; overview: ConnectorsOverview }) {
  const { def } = entry;
  const returnTo = settingsHref(site.id, null, "connectors");
  switch (def.key) {
    case "gsc":
    case "ga4": {

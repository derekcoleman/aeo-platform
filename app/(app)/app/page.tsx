        ...
        return (
          <section key={org.id} className="mb-10">
            <div className="mb-3 flex items-baseline justify-between">
              ...
              <span className="text-muted-foreground text-xs">{org.plan} · {own.length} project{own.length === 1 ? "" : "s"}{canManage(user, org.id) ? <> · <Link className="underline-offset-2 hover:underline" href={settingsHref(null, org.id) as Route}>settings</Link></> : null}</span>
            </div>
            {own.length === 0 ? (

...
import { acceptInvitesFor } from "@/lib/app/org";
...
export async function currentUser(sql: postgres.Sql = appDb()): Promise<SessionUser | null> {
  ...
  // A pending invite for this email becomes a membership the moment they sign in.
  await acceptInvitesFor(u.id, u.email, sql).catch(() => 0);
